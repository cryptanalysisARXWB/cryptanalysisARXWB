...
void encrypt(monomial *monomials, monomial *substituted, WORD_TYPE p[2], WORD_TYPE c[2]) {
    ...
    // printf("Input  : %" WORD_OUT_TYPE " %" WORD_OUT_TYPE "\n", c[0], c[1]);
    FIRST_EXPLICIT_ROUND(c[0], c[1]);
    // printf("Output : %" WORD_OUT_TYPE " %" WORD_OUT_TYPE "\n", c[0], c[1]);
    ...
    for (size_t r = 0; r < ROUNDS; r++) {
    	// printf("Round %zu first coeff %zu\n", r, coeffs_index);
    	// printf("Input  : %" WORD_OUT_TYPE " %" WORD_OUT_TYPE "\n", c[0], c[1]);
...
void getCoeffIndex(size_t allIndex[ROUNDS]){
	//Fake encryption just to get the index offset for each round
	//This should allow to query specific rounds without having to compute all rounds
...
void partialRoundEnc(WORD_TYPE p[2],
					 WORD_TYPE c[2],
					 size_t allIndex[ROUNDS],
					 size_t startingRound,
					 size_t nbRound){
    ...
    size_t bound = startingRound + nbRound;
    for (size_t r = startingRound; r < bound; r++) {
    	size_t coeffs_index = allIndex[r];
		// printf("Round %zu first coeff %zu\n", r, coeffs_index);
    	// printf("Input  : %" WORD_OUT_TYPE " %" WORD_OUT_TYPE "\n", c[0], c[1]);
        solve_implicit(monomials, substituted, c[0], c[1], &coeffs_index, &c[0], &c[1]);
        // printf("Output : %" WORD_OUT_TYPE " %" WORD_OUT_TYPE "\n", c[0], c[1]);
...
int main(int argc, char *argv[]) {
    ...
    // getCoeffIndex(allIndex);
    // for(int i = 0; i < ROUNDS; i++)
    // 	printf("%zu ",allIndex[i]);

    // p[0] = 0x694c;
    // p[1] = 0x694c;
    // partialRoundEnc(p,c,allIndex,0,21);

    // p[0] = 0x7d7d;
    // p[1] = 0xb158;
    // partialRoundEnc(p,c,allIndex,14,7);
